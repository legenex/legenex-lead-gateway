...
/**
 * Renders a StatusPill. For Error status, reveals the real failure reason
 * on hover (tooltip) and on click (popover). For other statuses, behaves
 * exactly like StatusPill.
 *
 * Props:
 *  - lead: the lead record
 *  - errorLogEntry: optional related ErrorLog entry
 *  - onOpenDetail: optional callback to open the lead detail (e.g. to the relevant trace)
 *  - size: pill size
 */
export default function ErrorStatusPill({ lead, errorLogEntry, onOpenDetail, size = 'sm' }) {
  const [open, setOpen] = useState(false);

  if (!lead || lead.final_status !== 'Error') {
    ...
    <TooltipProvider delayDuration={150}>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent
          align="start"
          sideOffset={6}
          className="w-[300px] p-3 bg-popover border-border text-[12px]"
        >
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0">
              ...
              {onOpenDetail && (
                <button
                  type="button"
                  onClick={() => { setOpen(false); onOpenDetail(lead, stage); }}
